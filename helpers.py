...
def normality_of_errors_assumption(model, features, label, p_value_thresh=0.05, plot=True):
    ...
    p_value = shapiro(df_results['Residuals'])[1]
...
def check_for_outliers(features):
    ...
    # Plotting
    plt.scatter(features[:, 0], features[:, 1], color='black', s=5, label='Data points')
    plt.scatter(features[y_pred == -1, 0], features[y_pred == -1, 1], color='red', s=20, label='Outliers')
...
def homoscedasticity(model,X,Y,p_value_thresh = 0.05):
    from statsmodels.stats.diagnostic import het_breuschpagan
    residuals = calculate_residuals(model,X,Y)['Residuals']
    X_with_constant = sm.add_constant(X)
    bp_test = het_breuschpagan(residuals,X_with_constant)

    return False if bp_test[1] <= p_value_thresh else True
...
def are_assumptions_satisfied(model, features, labels, p_value_thresh=0.05):
    ...
    linear_assumption(model, features, labels, plot=True)
    homoscedasticity_value = homoscedasticity (model,features,labels,p_value_thresh = 0.05)
    ...
    if homoscedasticity_value and autocorrelation is None and e_dist_type == 'equal':
        return True
    else:
        return False
